                ...
                priceMap.set(
                  ...
                  priceMap.get(seller) +
                    Number(item.Quantity) *
                      Number(sellerMap.get(seller).SellerPrice)
                );
              }
            }
          });
        ...
        if (minSeller !== undefined) {
          ...
          for (var i = 0; i < itemList.length; i++) {
            ...
            const item = allProducts.find(
              (data) => String(data._id) == String(itemList[i].productId)
            );
            ...
            itemDetails.push({
              ...
              totalPrice:
                Number(item.Sellers.get(minSeller).SellerPrice) *
                Number(itemList[i].Quantity),
            });
          }
          ...
          return res
            .status(404)
            .send({ message: "No seller available", itemDetails: itemDetails });
        }
      } else {
...
biddingRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .send({ message: "Success", createdBid: createdBid });
    } catch (err) {
      console.log("Internal server error\n", err);
      return res.status(500).send({ message: "Internal server error" });
    }
...
biddingRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    try {
      ...
      bids = bids.filter(
        (bid) => (Date.now() - bid.orderedAt) / (1000 * 60) < 180
      );
      ...
      if ((Date.now() - bid.orderedAt) / (1000 * 60) < 180) {
        ...
        if (price < bid.bids[bid.bids.length - 1].biddingPrice) {
          ...
          if (seller) {
            ...
            return res
              .status(200)
              .send({ message: "Success", updatedBid: updatedBid });
          } else {
  ...
  expressAsyncHandler(async (req, res) => {
    try {
      ...
      const seller = allSellers.find(
        (data) => String(data._id) == String(req.body.sellerId)
      );
      console.log(req.body);
      if (seller) {
        if (bid) {
          if (bid.city == seller.city) {
            ...
            for (var i = 0; i < bid.itemList.length; i++) {
              const product = allProducts.find(
                (p) => String(p._id) == String(bid.itemList[i].itemId)
              );
              resProducts.push({
                name: product.Name,
            ...
            for (var i = bid.bids.length - 1; i >= 0; i--) {
              const seller = allSellers.find(
                (s) => String(s._id) == String(bid.bids[i].sellerId)
              );
              resSellers.push({
            ...
            return res
              .status(400)
              .send({ message: "Seller city not within bidding zone" });
          }
        } else {
          return res
            .status(404)
